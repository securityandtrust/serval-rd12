...
public class GetBootloaderVersionNumberPacket extends StandardDataPacket {
    ...
    protected GetBootloaderVersionNumberPacket() {
        messageId = 0x09;
    }


    public static DataFrame parse(byte[] raw) {
        GetBootloaderVersionNumberPacket packet = new GetBootloaderVersionNumberPacket();
        packet.rawFrame = raw;
        ...
        if(packet.payloadLength == 8) {
            ...
            System.out.println("GetBootloaderVersionNumberPacket::Wrong payload length. Payload:" + packet.payloadLength + " expected:8\n" + Arrays.toString(raw));
        }
    ...
    public String toString() {
        return "Bootloader Version: " + getVersion() +  (ack_nack== Consts.ACK ? "(ACK)":"(NACK)");
    }
